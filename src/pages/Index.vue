  ...
  <q-page class="flex flex-center">
    <div v-if="hideUI === 'true'" class="text-center text-white text-h6" style="">
      ...
      <div class="text-title bg-black q-ma-sm q-pa-sm" style="font-size: 50px">
        {{ alert.user }} sent {{ alert.amount.split(' ')[0] }} <q-avatar size="md" class="q-ma-xs"><img :src="alert.amount.split(' ')[1] === 'HIVE' ? 'https://hive.ausbit.dev/statics/hive.svg' : 'https://hive.ausbit.dev/statics/hbd.svg'" /></q-avatar>
      </div>
      </transition>
      <transition appear :enter-active-class="anim1" :leave-active-class="anim2">
    ...
    <div v-else-if="hideUI !== 'true'">
      ...
      <div><a href="https://github.com/ausbitbank/donationalerts"><q-icon name="code" /> github.com/ausbitbank/donationalerts</a></div>
    </div>
  </q-page>
</template>
<script>
  ...
  setup () {
    ...
    return {
      ...
      async getTransfers () {
        ...
        var h = await client.call('condenser_api', 'get_account_history', [this.username, -1, 5, "4", null]) // Get last 5 transfer operations only
                    ...
                    if (amount >= this.minAlert) { // Amount is above minimum for alert
                      ...
                      if (amount >= this.minMemo && op.memo !== '') { // Amount is above minimum for memo
                        ...
                        if (amount >= this.minTTS) { // Amount is above minimum for tts
                          this.newDonation(this.alert.user, this.alert.amount, this.alert.memo)                        }
                      }
      ...
      newDonation(username, amount, memo) {
        this.showAlert = true
        this.textToSpeech(username + ' sent ' + amount)
        this.textToSpeech(memo)
      },
      textToSpeech(what) { window.speechSynthesis.speak(new SpeechSynthesisUtterance(what)) }
    }
  },
  computed: {
